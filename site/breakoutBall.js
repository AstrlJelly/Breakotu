const BALL_STATE = Object.freeze({
    DEFAULT : 0,
    BIG : 1,
    FAST : 2,
    SLOW : 4,
...
class BreakoutBall {
    ...
    hasCollidedWithRect(rect) {
        ...
        let rx = rect.getX(), ry = rect.getY();
        let rw = rect.getWidth(), rh = rect.getHeight();
    ...
    checkAllCollisions(ballX, ballY) {
        ...
        if (this.canBounce && ballY - BOUNCE_LENIENCY < paddle.getY()) {
            ...
            if (collision) {
                ...
                playOneShot("bounce")
                this.vel.y *= -1;
                let degrees = Math.atan2(this.vel.y, this.vel.x) * (180 / Math.PI);
                let ballNormalizedX = (ballX - paddle.getX()) / paddle.getWidth();
                degrees += MAX_BOUNCE_CHANGE * -(0.5 - ballNormalizedX);
                // -180 is left, 0 is right, -90 is straight up
                degrees = clamp(degrees, -180 + (90 - MAX_BOUNCE_ANGLE), -(90 - MAX_BOUNCE_ANGLE));
    ...
    reset() {
        let root = this;
        root.ball.setPosition(root.resetPos.x, root.resetPos.y);
    ...
    moveBallToCenter() {
        ...
        let endX = this.resetPos.x, endY = this.resetPos.y;

        let interval = setInterval(() => {
            if (time >= 1) { // end, clean up
    ...
    // ball : Circle, resetPos : Vector
    constructor(ball, resetPos) {
        this.ball = ball;
        this.resetPos = resetPos ?? new Vector(getWidth() / 2, getHeight() / 2);
        this.speed = 0.5;
        ...
        // };
        let root = this;
        setTimer(() => root.move(), FRAME_DELAY);
    }
}